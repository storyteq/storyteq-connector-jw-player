function StoryteqConnectorJwPlayer(parameters){var $vm=this;$vm.videoPlayerId=parameters.videoPlayerId;$vm.videoHash=$vm.getUrlParameter(parameters.videoParameterName);if(parameters.track){$vm.track=!0}
...
StoryteqConnectorJwPlayer.prototype.setJwPlayerInstance=function(response){var connector=this;var jwPlayerInstance=jwplayer(this.videoPlayerId);jwPlayerInstance.setup({file:this.videoUrl,image:this.posterUrl,events:{onComplete:function(){if(this.verbose){console.log('Video watched for 100% (complete)')}
...
StoryteqConnectorJwPlayer.prototype.getVideoData=function(){if(!this.videoHash||this.videoHash===undefined||this.videoHash===null){document.getElementById(this.videoPlayerId).innerHTML='No video hash has been given';return}
